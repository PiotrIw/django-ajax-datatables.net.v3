...
With django-ajax-datatables-net-v3, basically you have to provide a AjaxDatatableView-derived view
...
This package is a fork of `morlandi/django-ajax-datatable
<https://github.com/morlandi/django-ajax-datatable>`_, rewritten to target
**datatables.net v3**, which dropped its jQuery dependency. As part of that rewrite:

- jQuery and Bootstrap are gone, both from the library's own JS
  (``ajax_datatable/static/ajax_datatable/js/utils.js``) and from the demo app.
- The Python-side server API (``AjaxDatatableView``, ``column_defs``, settings, ...)
  is **unchanged** - the wire protocol between DataTables and the server was already
  stable across DataTables 1.x/2.x/3.x. Only client-side JS call sites need updating.
- Version numbering restarts at ``1.0.0``; Python floor is 3.10, Django floor is 5.2.
- The two example apps (``example/``, ``example_minimal/``) were merged into a
  single, dockerized ``demo/`` app.

See `MIGRATE_FROM_V4_TO_V1_CHECKLIST.rst <MIGRATE_FROM_V4_TO_V1_CHECKLIST.rst>`_ for
the full migration guide from the pre-v1.0.0 (jQuery-based) releases.

**Credits / Based on**

This package is a fork of the upstream project by Mario Orlandi (linked above),
itself inspired by `monnierj/django-datatables-server-side
<https://github.com/monnierj/django-datatables-server-side>`_. All credit for the
original design goes to Mario Orlandi; this fork is limited to the datatables.net v3 /
jQuery-free migration and accompanying modernization.
...
A working Django project demonstrating the library - Track/Album/Artist relations,
a custom-PK model, side filters, row details - lives in `demo/`. It's dockerized and
has no npm/Node build step: DataTables v3 is loaded straight from its CDN. See
`demo/README.rst <demo/README.rst>`_ for how to run it.
...
No PyPI package yet - install directly from GitHub:
...
Your base template should include what's required by `datatables.net` v3
(no jQuery, no Bootstrap needed), plus:

- /static/ajax_datatable/css/style.css
- /static/ajax_datatable/js/utils.js

Example (DataTables v3 from CDN):

.. code:: html

    {% block extrastyle %}

        <link rel="stylesheet" href="https://cdn.datatables.net/3.1.1/css/dataTables.dataTables.min.css">
...
If you'd rather style DataTables with Bootstrap 5 (or another framework),
DataTables' own `download builder <https://datatables.net/download/>`_ lets you
pick a styling-framework build; ``ajax_datatable``'s own CSS/JS have no opinion
on this and work with any of them.
...
Events are broadcast as native ``CustomEvent``\ s on the table element itself, with
the extra data carried in ``event.detail``::

    tableEl.dispatchEvent(new CustomEvent('foo', {detail: {one: 1, two: 2}}));

    tableEl.addEventListener('foo', function(event) {
        var one = event.detail.one, two = event.detail.two;
        ...
    });

Note that ``table`` in ``event.detail`` is the DataTables **API instance**
(what ``new DataTable(...)`` returns), not the ``<table>`` DOM element - use
``table.table().node()`` if you need the DOM node.
    ...
    AjaxDatatableViewUtils.initialize_table(
        ...
        url,
        {
            ...
            detail_callback: function(data, tr) {
                // 'data' is the <div class="row-details-wrapper"> DOM node built by
                // _load_row_details(); 'tr' is the row's <tr> DOM node.
                console.log('tr: %o', tr);
                console.log('data: %o', data);

                // for example: open it in your own modal/dialog widget instead of
                // showing it as an expanded table row
                document.querySelector('.modal-body').replaceChildren(data);
...
Note: DataTables v3's own default (non-jQuery) styling wraps the table in
``.dt-container`` (previous versions used ``.dataTables_wrapper``).

.. code:: html

    <style>
        .dt-container .dataTables_extraFooter {
...
a global date range filtering widget is provided, using plain
``<input type="date">`` elements (no datepicker library required):
...
You can fully replace the widget with your own by providing a custom
**fn_daterange_widget_initialize(tableEl, data, api, extraFilterState)** callback
at Module's initialization:
    ...
    AjaxDatatableViewUtils.init({
        ...
        fn_daterange_widget_initialize: function(tableEl, data, api, extraFilterState) {
            ...
            toolbar.innerHTML =
                ...
                '{% trans "From" %}: <input type="date" class="date_from">' +
                '&nbsp;&nbsp;' +
                '{% trans "To" %}: <input type="date" class="date_to">' +
                '</div>';
            toolbar.querySelectorAll('.date_from, .date_to').forEach(function(el) {
                el.addEventListener('change', function(event) {
                    // Annotate the table's filter state with values from the date widgets
...
Note that ``extraFilterState`` (a plain object holding ``date_from``/``date_to``)
replaces what used to be jQuery's ``.data()`` cache on the table element, and
``api`` is the DataTables API instance directly (``tableEl`` is the raw
``<table>`` DOM node).
...
where (this is what ``adjust_table_columns()`` itself does):
...
.. code:: javascript
    ...
    var tableEl = element.closest('table.dataTable');
...
.. code:: javascript
    ...
    new DataTable(tableEl).row(tr).invalidate().draw(false);
...
.. code:: javascript
        ...
        function onToggleQueueStatus(event) {
            ...
            var row_id = tr.id.substr(4);

            // Call the server via AJAX to process the record
            var url = sprintf('/{{LANGUAGE_CODE}}/j/product_order/%s/toggle_queue_status/', row_id);
...
.. code:: javascript
    ...
    fetch(url, {method: 'POST', headers: {'Accept': 'application/json'}}).then(function(response) {
        ...
        setTimeout(function() {
            tableEl.querySelector('#' + table_row_id).classList.add('updated');
        }, 200);
        setTimeout(function() {
            tableEl.querySelector('#' + table_row_id).classList.add('updated');
        }, 1000);
...
.. code:: javascript
    ...
    var onTableDraw = function (event) {
        ...
        var htmlTable = event.target;
        htmlTable.querySelectorAll('tr').forEach(function(row) {

            try {
                var cell = row.querySelector('td.error_counter');
...
.. code:: html
    ...
    // Subscribe "rowCallback" event
    document.querySelector('#datatable').addEventListener('rowCallback', function(event) {
        var row = event.detail.row, data = event.detail.data;
...
.. code:: javascript
    ...
    <script language="javascript">
        ...
        function set_row_alarm(element, task_id, value) {
            ...
            var tableEl = element.closest('table.dataTable');
            //console.log('table id: %o', tableEl.id);

            fetch(sprintf('/set_alarm/%s/%s/%d/', tableEl.id, task_id, value), {
